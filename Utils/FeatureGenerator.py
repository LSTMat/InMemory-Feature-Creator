...
def determine_field_type(variableCol: Dict[str, Column]) -> str:
    if variableCol[Table3ColumnName.COLUMN_IS_VARIABLE.value].get_value() == "True":
        return "variable"
    elif variableCol[Table3ColumnName.COLUMN_IS_PK.value].get_value() == "True":
        return "pk"
    else:
        return "entity"  
...
def add_ddVariables(DDModel_To_Update: ModelConfiguration, DataTUse: ModelConfiguration, table_name: str):
    tab = DataTUse.get_Table(table_name=TabName.TAB3.value)
    sequence_no = 1
...
def add_DDModelTableColumn(DDModel_To_Update: ModelConfiguration, DataTUse: ModelConfiguration, table_name: str):
    ...
    for row in tab.get_rows().values():
        columns = row.get_columns()
        Table_Model_info = DDModelTable_actual.filter_rows_by_condition(where_column_name="DisplayName", condition_value=columns[Table1ColumnName.COLUMN_TABLE_NAME.value].get_value())[0].get_columns()
        ...
        if variableCol[Table3ColumnName.COLUMN_IS_VARIABLE.value].get_value() == "True":
            ...
            for dd_variable_row in dd_variable_table.get_rows().values():
                if dd_variable_row.get_columns()["DisplayName"].get_value() == variableCol[Table3ColumnName.COLUMN_VARIABLE_NAME.value].get_value():
